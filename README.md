# Silicon Life Collective

A .NET 9 multi-agent collaboration platform where AI agents called **Silicon Beings** self-evolve through Roslyn dynamic compilation.

## 📖 Documentation

Choose your preferred language:

- **[English](docs/en/README.md)**
- **[简体中文](docs/zh-CN/README.md)**
- **[繁體中文](docs/zh-HK/README.md)**
- **[日本語](docs/ja-JP/README.md)**
- **[한국어](docs/ko-KR/README.md)**
- **[Español](docs/es-ES/README.md)**
- **[Čeština](docs/cs-CZ/README.md)**

## Quick Links

- [Architecture](docs/en/architecture.md) — System design and component architecture
- [Security](docs/en/security.md) — Permission model and executors
- [Roadmap](docs/en/roadmap.md) — Development plan
- [Getting Started](docs/en/getting-started.md) — Build and run guide

## Repository

- [GitHub](https://github.com/akimoto-akira/SiliconLifeCollective)
  - Detailed English documentation has been uploaded to the GitHub Wiki.
- [码云（中国镜像）](https://gitee.com/hoshinokennji/SiliconLifeCollective)
  - 中文版详细文档已经上传至码云 Wiki。

## License

Apache License 2.0 — See [LICENSE](LICENSE) for details.
