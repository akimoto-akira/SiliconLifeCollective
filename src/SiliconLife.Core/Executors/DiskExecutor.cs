...
public static class DiskExecutor
{
    ...
    private static ExecutorResult ExecuteReadFile(string path)
    {
        try
        {
            ...
            _logger.Info("Disk read: {Path}, size={Size}", path, content.Length);
            return ExecutorResult.Successful(content);
        }
        catch (Exception ex)
        {
            _logger.Error("Disk read failed: {Path}, {Exception}", ex, path);
    ...
    private static ExecutorResult ExecuteWriteFile(string path, Dictionary<string, object> parameters)
    {
        try
        {
            ...
            _logger.Info("Disk write: {Path}, size={Size}", path, content.Length);
            return ExecutorResult.Successful($"File written successfully: {path}");
        }
        catch (Exception ex)
        {
            _logger.Error("Disk write failed: {Path}, {Exception}", ex, path);
    ...
    private static ExecutorResult ExecuteAppendFile(string path, Dictionary<string, object> parameters)
    {
        try
        {
            ...
            _logger.Info("Disk append: {Path}, size={Size}", path, content.Length);
            return ExecutorResult.Successful($"Content appended to file: {path}");
        }
        catch (Exception ex)
        {
            _logger.Error("Disk append failed: {Path}, {Exception}", ex, path);
    ...
    private static ExecutorResult ExecuteDeleteFile(string path)
    {
        try
        {
            ...
            {
                File.Delete(path);
                _logger.Info("Disk delete: {Path}", path);
            ...
            if (Directory.Exists(path))
            {
                ...
                _logger.Info("Disk delete directory: {Path}", path);
        ...
        {
            _logger.Error("Disk delete failed: {Path}, {Exception}", ex, path);
    ...
    private static ExecutorResult ExecuteListDirectory(string path)
    {
        try
        {
            ...
            _logger.Info("Disk list directory: {Path}, entries={Count}", path, entries.Count);
            return ExecutorResult.Successful(result);
        }
        catch (Exception ex)
        {
            _logger.Error("Disk list directory failed: {Path}, {Exception}", ex, path);
    ...
    private static ExecutorResult ExecuteCreateDirectory(string path)
    {
        try
        {
            ...
            _logger.Info("Disk create directory: {Path}", path);
            return ExecutorResult.Successful($"Directory created: {path}");
        }
        catch (Exception ex)
        {
            _logger.Error("Disk create directory failed: {Path}, {Exception}", ex, path);
    ...
    private static ExecutorResult ExecuteGetFileInfo(string path)
    {
        try
        {
            if (File.Exists(path))
            {
                ...
                _logger.Info("Disk get file info: {Path}, size={Size}", path, fi.Length);
            ...
            if (Directory.Exists(path))
            {
                ...
                _logger.Info("Disk get directory info: {Path}, files={Files}, dirs={Dirs}", path, fileCount, dirCount);
                return ExecutorResult.Successful(info);
            }
        ...
        {
            _logger.Error("Disk get file info failed: {Path}, {Exception}", ex, path);
