...
/// <summary>
/// Implements the Coptic calendar system.
/// Epoch JDN: 1825030. 13 months: first 12 have 30 days, 13th has 5 days (6 in leap years).
/// Leap year: year % 4 == 0.
/// </summary>
public class CopticCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    private const int EpochJdn = 1825030;
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "coptic";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarCopticName;
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatCopticYear(value),
            Month  => loc.GetCopticMonthName(value),
            Day    => loc.FormatCopticDay(value),
    ...
    /// <summary>
    /// Returns a localized human-readable string via <see cref="DefaultLocalizationBase.LocalizeCopticDate"/>.
    /// </summary>
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeCopticDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
