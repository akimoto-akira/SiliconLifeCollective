...
/// <summary>
/// Implements the Saka Era calendar system.
/// Epoch: JDN 1749630 (March 22, 78 CE). Saka year 0 = 78 CE.
/// </summary>
public class SakaCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    private const int Epoch = 1749630;
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "saka";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarSakaName;
    ...
    {
        int sakaYear = Math.Max(0, (jdn - Epoch) / 365);
        while (GetSakaNewYearJdn(sakaYear + 1) <= jdn) sakaYear++;
        while (sakaYear > 0 && GetSakaNewYearJdn(sakaYear) > jdn) sakaYear--;
        int month = 1;
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatSakaYear(value),
            Month  => loc.GetIndianMonthName(value),
            Day    => loc.FormatSakaDay(value),
    ...
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeSakaDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
