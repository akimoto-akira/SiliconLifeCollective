...
/// <summary>
/// Implements the Juche calendar system used in North Korea.
/// Juche year = Gregorian year - 1911.
/// </summary>
public class JucheCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    private const int YearOffset = 1911;
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "juche";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarJucheName;
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatJucheYear(value),
            Month  => loc.GetJucheMonthName(value),
            Day    => loc.FormatJucheDay(value),
    ...
    /// <summary>
    /// Returns a localized human-readable string via <see cref="DefaultLocalizationBase.LocalizeJucheDate"/>.
    /// </summary>
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeJucheDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
