...
/// <summary>
/// Implements the Javanese Islamic calendar system.
/// Epoch: JDN 1948440 (same as Islamic). 30-year cycle.
/// </summary>
public class JavaneseCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "javanese";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarJavaneseName;
    ...
    {
        int r = year % 30;
        return r is 2 or 5 or 7 or 10 or 13 or 16 or 18 or 21 or 24 or 26 or 29;
    }
    ...
    private static int ToJdn(int year, int month, int day)
    {
        ...
        return Epoch + (year - 1) * 354 + CountLeapYears(year - 1) + daysInCompleteMonths + day - 1;
    }

    private static (int year, int month, int day) FromJdn(int jdn)
    {
        int year = Math.Max(1, (jdn - Epoch) / 354 + 1);
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatJavaneseYear(value),
            Month  => loc.GetJavaneseMonthName(value),
            Day    => loc.FormatJavaneseDay(value),
    ...
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeJavaneseDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
