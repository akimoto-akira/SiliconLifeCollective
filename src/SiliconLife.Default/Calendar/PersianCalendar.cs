...
/// <summary>
/// Implements the Persian (Solar Hijri / Jalali) calendar system.
/// Epoch: JDN 1948277.
/// </summary>
public class PersianCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    private const int Epoch = 1948277;
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "persian";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarPersianName;
    ...
    {
        int r = year % 128 % 33;
        return r is 1 or 5 or 9 or 13 or 17 or 20 or 22 or 26 or 30;
    }
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatPersianYear(value),
            Month  => loc.GetPersianMonthName(value),
            Day    => loc.FormatPersianDay(value),
    ...
    /// <summary>
    /// Returns a localized human-readable string via <see cref="DefaultLocalizationBase.LocalizePersianDate"/>.
    /// </summary>
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizePersianDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
