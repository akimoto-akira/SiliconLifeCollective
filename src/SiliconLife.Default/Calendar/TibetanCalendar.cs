...
/// <summary>
/// Implements a simplified Tibetan lunisolar calendar.
/// Epoch: JDN 1676438. Approximates using 354.367 days/year and 29.53059 days/month.
/// </summary>
public class TibetanCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    private const int    Epoch        = 1676438;
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "tibetan";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarTibetanName;
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatTibetanYear(value),
            Month  => loc.FormatTibetanMonth(value),
            Day    => loc.FormatTibetanDay(value),
    ...
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeTibetanDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
