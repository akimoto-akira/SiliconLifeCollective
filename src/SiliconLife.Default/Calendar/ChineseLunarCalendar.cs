...
/// <summary>
/// Implements the Chinese Lunar (Nong Li) calendar system.
/// Supports years 1900–2100 using the standard lunar data table.
/// </summary>
public class ChineseLunarCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    // Lunar data for years 1900–2100
    // Bits 0-3: leap month (0=none); bits 4-15: days in months 1-12 (0=29,1=30); bit 16: days in leap month
    ...
    private const int Lunar1900Jdn = 2415051; // 1900-01-31
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "chinese_lunar";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarChineseLunarName;
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Month  => loc.GetChineseLunarMonthName(value),
            Day    => loc.GetChineseLunarDayName(value),
    ...
    /// <summary>
    /// Formats components as <c>yyyy-MM-dd HH:mm:ss</c> or <c>yyyy-LMM-dd HH:mm:ss</c> for leap months.
    /// </summary>
    ...
    /// <summary>
    /// Returns a localized human-readable string via <see cref="DefaultLocalizationBase.LocalizeChineseLunarDate"/>.
    /// </summary>
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeChineseLunarDate(c[Year], c[Month], c[Day], isLeap, h, mi, s);
    }
}
