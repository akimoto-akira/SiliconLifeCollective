...
/// <summary>
/// Implements the Khmer calendar system (Buddhist Era, as used in Cambodia).
/// Khmer year = Gregorian year + 543.
/// </summary>
public class KhmerCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    private const int YearOffset = 543;
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "khmer";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarKhmerName;
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatKhmerYear(value),
            Month  => loc.GetGregorianMonthName(value),
            Day    => loc.FormatKhmerDay(value),
    ...
    /// <summary>
    /// Returns a localized human-readable string via <see cref="DefaultLocalizationBase.LocalizeKhmerDate"/>.
    /// </summary>
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeKhmerDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
