// Copyright (c) 2026 Hoshino Kennji
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

namespace SiliconLife.Default;

/// <summary>
/// Implements the Buddhist Era (BE) calendar system.
/// Buddhist year = Gregorian year + 543.
/// </summary>
public class BuddhistCalendar : CalendarBase
{
    private const int YearOffset = 543;

    private const string Year    = "year";
    private const string Month   = "month";
    private const string Day     = "day";
    private const string Hour    = "hour";
    private const string Minute  = "minute";
    private const string Second  = "second";

    private readonly DefaultLocalizationBase _loc;

    public BuddhistCalendar(DefaultLocalizationBase loc) => _loc = loc;

    public override string CalendarId   => "buddhist";
    public override string CalendarName => _loc.CalendarBuddhistName;

    public override Dictionary<string, string> GetComponents() => new()
    {
        { Year,   _loc.CalendarComponentYear   },
        { Month,  _loc.CalendarComponentMonth  },
        { Day,    _loc.CalendarComponentDay    },
        { Hour,   _loc.CalendarComponentHour   },
        { Minute, _loc.CalendarComponentMinute },
        { Second, _loc.CalendarComponentSecond }
    };

    public override Dictionary<string, int> FromDateTime(DateTime dt) => new()
    {
        { Year,   dt.Year + YearOffset },
        { Month,  dt.Month             },
        { Day,    dt.Day               },
        { Hour,   dt.Hour              },
        { Minute, dt.Minute            },
        { Second, dt.Second            }
    };

    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        result = componentId switch
        {
            Year   => _loc.FormatBuddhistYear(value),
            Month  => _loc.GetBuddhistMonthName(value),
            Day    => _loc.FormatBuddhistDay(value),
            Hour   => _loc.FormatGregorianHour(value),
            Minute => _loc.FormatGregorianMinute(value),
            Second => _loc.FormatGregorianSecond(value),
            _      => null
        };
        return result != null;
    }

    public override string Format(Dictionary<string, int> c)
    {
        var h  = c.GetValueOrDefault(Hour,   0);
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return $"{c[Year]:D4}-{c[Month]:D2}-{c[Day]:D2} {h:D2}:{mi:D2}:{s:D2}";
    }

    public override Dictionary<string, int> Parse(string value)
    {
        var formats = new[] { "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd" };
        var dt = DateTime.ParseExact(value, formats,
            System.Globalization.CultureInfo.InvariantCulture,
            System.Globalization.DateTimeStyles.None);
        return FromDateTime(dt);
    }

    public override string Localize(Dictionary<string, int> c)
    {
        var h  = c.GetValueOrDefault(Hour,   0);
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return _loc.LocalizeBuddhistDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
