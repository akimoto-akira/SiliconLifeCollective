...
/// <summary>
/// Implements the Republic of China (ROC / Minguo) calendar system.
/// ROC year = Gregorian year - 1911.
/// </summary>
public class RepublicOfChinaCalendar(DefaultLocalizationBase loc) : CalendarBase
{
    ...
    /// <inheritdoc/>
    public override string CalendarId   => "roc";

    /// <inheritdoc/>
    public override string CalendarName => loc.CalendarRocName;
    ...
    public override bool TryGetComponentValueLocalization(string componentId, int value, out string? result)
    {
        ...
        {
            Year   => loc.FormatRocYear(value),
            Month  => loc.GetRocMonthName(value),
            Day    => loc.FormatRocDay(value),
    ...
    /// <summary>
    /// Returns a localized human-readable string via <see cref="DefaultLocalizationBase.LocalizeRocDate"/>.
    /// </summary>
    public override string Localize(Dictionary<string, int> c)
    {
        ...
        var mi = c.GetValueOrDefault(Minute, 0);
        var s  = c.GetValueOrDefault(Second, 0);
        return loc.LocalizeRocDate(c[Year], c[Month], c[Day], h, mi, s);
    }
}
