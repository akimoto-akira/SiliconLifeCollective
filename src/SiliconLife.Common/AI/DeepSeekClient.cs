...
internal class DeepSeekRequest
{
    ...
    public DeepSeekThinkingConfig? Thinking { get; set; }
    [JsonPropertyName("reasoning_effort")]
    public string? ReasoningEffort { get; set; }
}

internal class DeepSeekThinkingConfig
{
...
internal class DeepSeekToolCall
{
    [JsonPropertyName("id")]
...
internal class DeepSeekToolCallFunction
{
    public string Name { get; set; } = string.Empty;
    public string? Arguments { get; set; }
}

internal class DeepSeekTool
{
    public string Type { get; set; } = "function";
    public DeepSeekToolFunction? Function { get; set; }
}

internal class DeepSeekToolFunction
{
    public string Name { get; set; } = string.Empty;
...
/// <summary>
/// DeepSeek (深度求索) AI client implementation.
...
{
    private static readonly ILogger _logger = LogManager.Instance.GetLogger<DeepSeekClient>();
    ...
    internal static int? GetContextWindowTokensForModel(string? modelName)
    {
        ...
        if (lower.Contains("deepseek-v4")) return MaxContextWindowTokens;
    ...
    public async Task<AIResponse> ChatAsync(AIRequest request)
    {
        try
        {
            ...
            _logger.Info(null, "DeepSeek request: model={0}, messages={1}, hasTools={2}",
    ...
    {
        string model = string.IsNullOrEmpty(request.Model) ? DefaultModel : request.Model;
        _logger.Info(null, "DeepSeek stream started: model={0}", model);
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.Error(null, "DeepSeek stream HTTP error: {0} {1}", (int)response.StatusCode, errorBody);
                errorResponse = AIResponse.Failed($"HTTP {(int)response.StatusCode}: {errorBody}");
            }
        }
        catch (HttpRequestException ex)
        {
            _logger.Error(null, "DeepSeek stream connection error: {0}", ex.Message);
            errorResponse = AIResponse.Failed($"Connection error: {ex.Message}");
        }
        catch (OperationCanceledException) { _logger.Debug(null, "DeepSeek stream cancelled"); yield break; }
        ...
        while (!reader.EndOfStream)
        {
            ...
            if (delta.TryGetProperty("tool_calls", out var tcDelta))
            {
                ...
                catch (Exception ex) { _logger.Warn(null, "DeepSeek stream: failed to parse tool_calls chunk: {0}", ex.Message); }
            }
            ...
            if (finishReason != null)
            {
                ...
                _logger.Info(null, "DeepSeek stream completed: model={0}, totalTokens={1}", model, chunk.TotalTokens);
            }
            yield return chunk;
        }
    }
    ...
    private string BuildRequestBody(AIRequest request, string model, bool stream)
    {
        var req = new DeepSeekRequest
        {
            ...
            Stream = stream,
            Thinking = new DeepSeekThinkingConfig { Type = _thinkingEnabled ? "enabled" : "disabled" },
            ReasoningEffort = _thinkingEnabled ? _reasoningEffort : null
        };

        if (request.Tools != null && request.Tools.Count > 0)
        {
            req.Tools = request.Tools.Select(t => new DeepSeekTool
            {
                Type = "function",
                Function = new DeepSeekToolFunction { Name = t.Name, Description = t.Description, Parameters = t.Parameters }
            }).ToList();
        }
        return JsonSerializer.Serialize(req, _jsonOptions);
    }

    private static List<DeepSeekMessage> MapMessages(List<ChatMessage> messages)
    {
        ...
        if (systemContents.Count > 0)
            result.Add(new DeepSeekMessage { Role = "system", Content = string.Join("\n", systemContents) });

        foreach (var msg in messages)
        {
            if (msg.Role == MessageRole.System) continue;
            var dsMsg = new DeepSeekMessage
            {
            ...
            if (msg.Role == MessageRole.Assistant && !string.IsNullOrEmpty(msg.ToolCallsJson))
            {
                try
                {
                    ...
                    {
                        dsMsg.ToolCalls = toolCalls.Select(tc => new DeepSeekToolCall
                        {
                            Id = tc.Id, Type = "function",
                            Function = new DeepSeekToolCallFunction { Name = tc.Name, Arguments = JsonSerializer.Serialize(tc.Arguments) }
                        }).ToList();
                    }
                }
                catch { }
    ...
    private AIResponse ParseChatResponse(string json)
    {
        ...
        {
            _logger.Error(null, "DeepSeek response parse error: {0}", ex.Message);
