...
internal class SiliconFlowRequest
{
    public string Model { get; set; } = string.Empty;
    public List<SiliconFlowMessage> Messages { get; set; } = new();
    [JsonPropertyName("tools")]
    public List<SiliconFlowTool>? Tools { get; set; }
    public bool Stream { get; set; } = false;
    [JsonPropertyName("enable_thinking")]
    public bool? EnableThinking { get; set; }
}

internal class SiliconFlowMessage
{
    ...
    public List<SiliconFlowToolCall>? ToolCalls { get; set; }
    [JsonPropertyName("tool_call_id")]
    public string? ToolCallId { get; set; }
}

internal class SiliconFlowToolCall
{
    [JsonPropertyName("id")]
    public string? Id { get; set; }
    public string Type { get; set; } = "function";
    public SiliconFlowToolCallFunction? Function { get; set; }
}

internal class SiliconFlowToolCallFunction
{
    public string Name { get; set; } = string.Empty;
    public string? Arguments { get; set; }
}

internal class SiliconFlowTool
{
    public string Type { get; set; } = "function";
    public SiliconFlowToolFunction? Function { get; set; }
}

internal class SiliconFlowToolFunction
{
    public string Name { get; set; } = string.Empty;
...
/// <summary>
/// SiliconFlow (硅基流动 SiliconCloud) AI client implementation.
/// Uses OpenAI-compatible API format with Bearer token authentication.
/// Aggregates 100+ open-source models from multiple vendors.
/// Supports tool calling, streaming, and reasoning content.
/// </summary>
public class SiliconFlowClient : IAIClient
{
    private static readonly ILogger _logger = LogManager.Instance.GetLogger<SiliconFlowClient>();
    ...
    public SiliconFlowClient(string apiKey, string endpoint,
        string defaultModel = "deepseek-ai/DeepSeek-V3.2",
    ...
    internal static int? GetContextWindowTokensForModel(string? modelName)
    {
        ...
        if (m.StartsWith("pro/")) m = m[4..];
        if (m.Contains("deepseek-v4")) return 1048576;
        if (m.Contains("deepseek-v3")) return 167936;
        if (m.Contains("qwen3.6") || m.Contains("qwen3.5")) return 268288;
        if (m.Contains("glm-5.2")) return 1048576;
        if (m.Contains("glm-5.1") || m.Contains("glm-5") || m.Contains("glm-4.7") || m.Contains("glm-4.6")) return 204800;
        if (m.Contains("kimi-k2")) return 268288;
        if (m.Contains("gpt-oss")) return 131072;
        if (m.Contains("gemma-4")) return 268288;
        if (m.Contains("minimax-m3")) return 1048576;
        if (m.Contains("minimax-m2")) return 196608;
        return null;
    }
    ...
    internal static bool? GetSupportsVisionForModel(string? modelName)
    {
        ...
        string m = modelName.ToLowerInvariant();
        if (m.StartsWith("pro/")) m = m[4..];
        if (m.Contains("qwen3.5-9b") || m.Contains("qwen3.6") || m.Contains("qwen3-vl")) return true;
        if (m.Contains("glm-4.6v") || m.Contains("glm-5v")) return true;
        if (m.Contains("kimi-k2.5") || m.Contains("kimi-k2.6") || m.Contains("kimi-k2.7")) return true;
        if (m.Contains("vision")) return true;
        if (m.Contains("deepseek") || m.Contains("gpt-oss") || m.Contains("gemma")) return false;
        return null;
    }
    ...
    public async Task<AIResponse> ChatAsync(AIRequest request)
    {
        try
        {
            ...
            _logger.Info(null, "SiliconFlow request: model={0}, messages={1}, hasTools={2}",
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.Error(null, "SiliconFlow HTTP error: {0} {1}", (int)response.StatusCode, errorBody);
                return AIResponse.Failed($"HTTP {(int)response.StatusCode}: {errorBody}");
            }

            string json = await response.Content.ReadAsStringAsync();
            AIResponse result = ParseChatResponse(json);
            _logger.Info(null, "SiliconFlow response: model={0}, tokens={1}/{2}/{3}, hasToolCalls={4}",
                model, result.PromptTokens, result.CompletionTokens, result.TotalTokens, result.HasToolCalls);
            return result;
        }
        catch (HttpRequestException ex)
        {
            _logger.Error(null, "SiliconFlow connection error: {0}", ex.Message);
            return AIResponse.Failed($"Connection error: {ex.Message}");
        }
        catch (TaskCanceledException ex)
        {
            _logger.Warn(null, "SiliconFlow request timeout: {0}", ex.Message);
            return AIResponse.Failed($"Request timeout: {ex.Message}");
        }
        catch (Exception ex)
        {
            _logger.Error(null, "SiliconFlow request failed: {0}", ex.Message);
    ...
    {
        string model = string.IsNullOrEmpty(request.Model) ? DefaultModel : request.Model;
        _logger.Info(null, "SiliconFlow stream started: model={0}", model);
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.Error(null, "SiliconFlow stream HTTP error: {0} {1}", (int)response.StatusCode, errorBody);
                errorResponse = AIResponse.Failed($"HTTP {(int)response.StatusCode}: {errorBody}");
            }
        }
        catch (HttpRequestException ex)
        {
            _logger.Error(null, "SiliconFlow stream connection error: {0}", ex.Message);
            errorResponse = AIResponse.Failed($"Connection error: {ex.Message}");
        }
        catch (OperationCanceledException) { _logger.Debug(null, "SiliconFlow stream cancelled"); yield break; }
        ...
        while (!reader.EndOfStream)
        {
            ...
            if (delta.TryGetProperty("tool_calls", out var tcDelta))
            {
                ...
                catch (Exception ex) { _logger.Warn(null, "SiliconFlow stream: failed to parse tool_calls chunk: {0}", ex.Message); }
            }
            ...
            if (finishReason != null)
            {
                ...
                _logger.Info(null, "SiliconFlow stream completed: model={0}, totalTokens={1}", model, chunk.TotalTokens);
            }
            yield return chunk;
        }
    }
    ...
    {
        var req = new SiliconFlowRequest
        {
            Model = model,
        ...
        {
            req.Tools = request.Tools.Select(t => new SiliconFlowTool
            {
                Type = "function",
                Function = new SiliconFlowToolFunction { Name = t.Name, Description = t.Description, Parameters = t.Parameters }
            }).ToList();
        }
        return JsonSerializer.Serialize(req, _jsonOptions);
    }

    private static List<SiliconFlowMessage> MapMessages(List<ChatMessage> messages)
    {
        var result = new List<SiliconFlowMessage>();
        ...
        if (systemContents.Count > 0)
            result.Add(new SiliconFlowMessage { Role = "system", Content = string.Join("\n", systemContents) });

        foreach (var msg in messages)
        {
            if (msg.Role == MessageRole.System) continue;
            var sfMsg = new SiliconFlowMessage
            {
            ...
            if (msg.Role == MessageRole.Assistant && !string.IsNullOrEmpty(msg.ToolCallsJson))
            {
                try
                {
                    ...
                    {
                        sfMsg.ToolCalls = toolCalls.Select(tc => new SiliconFlowToolCall
                        {
                            Id = tc.Id, Type = "function",
                            Function = new SiliconFlowToolCallFunction { Name = tc.Name, Arguments = JsonSerializer.Serialize(tc.Arguments) }
                        }).ToList();
                    }
                }
                catch { }
            ...
            if (msg.Role == MessageRole.Tool && !string.IsNullOrEmpty(msg.ToolCallId))
                sfMsg.ToolCallId = msg.ToolCallId;
            result.Add(sfMsg);
        }
        return result;
    }
    ...
    private AIResponse ParseChatResponse(string json)
    {
        ...
        {
            _logger.Error(null, "SiliconFlow response parse error: {0}", ex.Message);
