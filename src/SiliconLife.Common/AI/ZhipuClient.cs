...
internal class ZhipuRequest
{
    public string Model { get; set; } = string.Empty;
    public List<ZhipuMessage> Messages { get; set; } = new();
    [JsonPropertyName("tools")]
    public List<ZhipuTool>? Tools { get; set; }
    public bool Stream { get; set; } = false;
    [JsonPropertyName("thinking")]
    public ZhipuThinkingConfig? Thinking { get; set; }
}

internal class ZhipuThinkingConfig
{
...
internal class ZhipuMessage
{
    ...
    public List<ZhipuToolCall>? ToolCalls { get; set; }
    [JsonPropertyName("tool_call_id")]
    public string? ToolCallId { get; set; }
}

internal class ZhipuToolCall
{
    [JsonPropertyName("id")]
    public string? Id { get; set; }
    public string Type { get; set; } = "function";
    public ZhipuToolCallFunction? Function { get; set; }
}

internal class ZhipuToolCallFunction
{
    public string Name { get; set; } = string.Empty;
    public string? Arguments { get; set; }
}

internal class ZhipuTool
{
    public string Type { get; set; } = "function";
    public ZhipuToolFunction? Function { get; set; }
}

internal class ZhipuToolFunction
{
    public string Name { get; set; } = string.Empty;
...
/// <summary>
/// Zhipu AI (智谱 GLM) client implementation.
/// Uses OpenAI-compatible API format with Bearer token authentication.
/// Supports tool calling, streaming, thinking mode, and free model (glm-4-flash).
/// </summary>
public class ZhipuClient : IAIClient
{
    private static readonly ILogger _logger = LogManager.Instance.GetLogger<ZhipuClient>();
    ...
    public ZhipuClient(string apiKey, string endpoint,
    ...
    internal static int? GetContextWindowTokensForModel(string? modelName)
    {
        ...
        if (lower.Contains("glm-5.2") || lower.Contains("glm-4-long")) return 1048576;
        if (lower.Contains("glm-4.7") || lower.Contains("glm-4.6")) return 204800;
        if (lower.StartsWith("glm-4") || lower.StartsWith("glm-5")) return 131072;
        return null;
    }
    ...
    internal static bool? GetSupportsVisionForModel(string? modelName)
    {
        if (string.IsNullOrEmpty(modelName)) return null;
        string lower = modelName.ToLowerInvariant();
        if (lower.StartsWith("glm-4v") || lower.StartsWith("glm-5v") || lower.Contains("vision")) return true;
        if (lower.StartsWith("glm-4") || lower.StartsWith("glm-5")) return false;
        return null;
    }
    ...
    public async Task<AIResponse> ChatAsync(AIRequest request)
    {
        try
        {
            ...
            _logger.Info(null, "Zhipu request: model={0}, messages={1}, hasTools={2}",
    ...
    {
        string model = string.IsNullOrEmpty(request.Model) ? DefaultModel : request.Model;
        _logger.Info(null, "Zhipu stream started: model={0}", model);
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.Error(null, "Zhipu stream HTTP error: {0} {1}", (int)response.StatusCode, errorBody);
                errorResponse = AIResponse.Failed($"HTTP {(int)response.StatusCode}: {errorBody}");
            }
        }
        catch (HttpRequestException ex)
        {
            _logger.Error(null, "Zhipu stream connection error: {0}", ex.Message);
            errorResponse = AIResponse.Failed($"Connection error: {ex.Message}");
        }
        catch (OperationCanceledException) { _logger.Debug(null, "Zhipu stream cancelled"); yield break; }
        ...
        while (!reader.EndOfStream)
        {
            ...
            if (delta.TryGetProperty("tool_calls", out var tcDelta))
            {
                ...
                catch (Exception ex) { _logger.Warn(null, "Zhipu stream: failed to parse tool_calls chunk: {0}", ex.Message); }
            }
            ...
            if (finishReason != null)
            {
                ...
                _logger.Info(null, "Zhipu stream completed: model={0}, totalTokens={1}", model, chunk.TotalTokens);
            }
            yield return chunk;
        }
    }
    ...
    private string BuildRequestBody(AIRequest request, string model, bool stream)
    {
        ...
        // Enable thinking only for GLM-5 series
        string lower = model.ToLowerInvariant();
        if (lower.StartsWith("glm-5") && _thinkingEnabled)
            req.Thinking = new ZhipuThinkingConfig { Type = "enabled" };

        if (request.Tools != null && request.Tools.Count > 0)
        {
            req.Tools = request.Tools.Select(t => new ZhipuTool
            {
                Type = "function",
                Function = new ZhipuToolFunction { Name = t.Name, Description = t.Description, Parameters = t.Parameters }
            }).ToList();
        }
        return JsonSerializer.Serialize(req, _jsonOptions);
    }

    private static List<ZhipuMessage> MapMessages(List<ChatMessage> messages)
    {
        var result = new List<ZhipuMessage>();
        ...
        if (systemContents.Count > 0)
            result.Add(new ZhipuMessage { Role = "system", Content = string.Join("\n", systemContents) });

        foreach (var msg in messages)
        {
            if (msg.Role == MessageRole.System) continue;
            var zMsg = new ZhipuMessage
            {
            ...
            if (msg.Role == MessageRole.Assistant && !string.IsNullOrEmpty(msg.ToolCallsJson))
            {
                try
                {
                    ...
                    {
                        zMsg.ToolCalls = toolCalls.Select(tc => new ZhipuToolCall
                        {
                            Id = tc.Id, Type = "function",
                            Function = new ZhipuToolCallFunction { Name = tc.Name, Arguments = JsonSerializer.Serialize(tc.Arguments) }
                        }).ToList();
                    }
                }
                catch { }
            ...
            if (msg.Role == MessageRole.Tool && !string.IsNullOrEmpty(msg.ToolCallId))
                zMsg.ToolCallId = msg.ToolCallId;
            result.Add(zMsg);
        }
        return result;
    }
    ...
    private AIResponse ParseChatResponse(string json)
    {
        ...
        {
            _logger.Error(null, "Zhipu response parse error: {0}", ex.Message);
