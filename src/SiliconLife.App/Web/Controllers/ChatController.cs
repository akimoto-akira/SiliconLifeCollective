...
public class ChatController : Controller
{
    ...
    private void StopThinking()
    {
        try
        {
            ...
            if (cancelled)
            {
                _logger.Info(null, "StopThinking: Stream stopped for channel {0}", channelId);
            }
    ...
    private void UploadFile()
    {
        try
        {
            ...
            // File upload processed, notification handled by core chat system
    ...
    private void UploadFileFromMultipart()
    {
        try
        {
            ...
            // Store uploaded file using storage factory
            var fileName = SanitizeFileName(fileData.FileName ?? "uploaded_file");
            var uploadKey = $"uploads/{Guid.NewGuid()}/{fileName}";
            var storage = ServiceLocator.Instance.StorageFactory?.Invoke("");
            if (storage != null)
            {
                storage.Write(uploadKey + ".json", fileData.Data);
            }
