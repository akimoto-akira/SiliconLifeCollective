...
/// <summary>
/// View for the tool action permission configuration page.
/// Displays a per-tool action matrix with toggles, template quick-apply,
/// and tool-level select/deselect-all controls.
/// </summary>
public class ToolPermissionView : ViewBase
{
    public override string Render(object model)
    {
        var vm = model as ToolPermissionViewModel;
        if (vm == null) return string.Empty;

        var body = RenderBody(vm);
        return RenderPage(vm.Skin, $"{vm.BeingName} - {vm.Localization.ToolAuthPageTitle}", "beings",
            vm.Localization, body, GetScripts(vm), GetStyles(), helpTopicId: "being-management");
    }

    private static H RenderBody(ToolPermissionViewModel vm)
    {
        var loc = vm.Localization;

        return H.Div(
            H.Div(
                H.A("← " + loc.BeingsBackToList).Href("/beings").Class("tool-auth-back-link"),
                H.H1(vm.BeingName).Class("tool-auth-title"),
                H.P(loc.ToolAuthPageHeader).Class("tool-auth-subtitle")
            ).Class("tool-auth-header"),
            H.Div().Id("alert").Class("alert"),
            H.Div(
                H.Create("label", loc.ToolAuthTemplateLabel).Class("template-label"),
                H.Create("select",
                    H.Create("option", "").Attr("value", "").Text("--"),
                    H.Create("option", loc.ToolAuthNoRestrictions).Attr("value", "full_control"),
                    H.Create("option", "Only Chat / 只聊天").Attr("value", "chat_only"),
                    H.Create("option", "Task Execution / 只执行任务").Attr("value", "task_execution")
                ).Id("templateSelect").Class("template-select").Attr("onchange", "applyTemplate()"),
                H.Create("button", "▶").Class("btn btn-sm").Attr("type", "button").Attr("onclick", "applyTemplate()")
            ).Class("template-row"),
    ...
    private static JsSyntax GetScripts(ToolPermissionViewModel vm)
    {
        var loc = vm.Localization;
        var js = Js.Block();
        ...
        var loadBody = Js.Block()
            .Add(() => Js.Id(() => "fetch").Invoke(() => Js.Str(() => "/api/beings/tool-permissions?beingId=").Op(() => "+", () => (JsSyntax)Js.Id(() => "beingId")))
        ...
        // applyTemplate
        BuildApplyTemplate(js);
        // showAlert (used by applyTemplate)
        BuildShowAlert(js);
        // saveDialog
    ...
    private static void BuildSavePermissions(JsBlock js, DefaultLocalizationBase loc)
    {
        ...
        var saveBody = Js.Block()
            ...
            .Add(() => Js.Id(() => "fetch").Invoke(
                () => Js.Str(() => "/api/beings/tool-permissions?beingId=").Op(() => "+", () => (JsSyntax)Js.Id(() => "beingId")),
                () => Js.Obj()
    ...
    private static void BuildApplyTemplate(JsBlock js)
    {
        var applyDataBlock = Js.Block()
            .Add(() => Js.If(() => new List<(JsSyntax?, List<JsSyntax>)>
            {
                { (Js.Id(() => "data").Prop(() => "success"), new List<JsSyntax>
                    { Js.Id(() => "loadPermissions").Invoke().Stmt() }
                )},
                { (null, new List<JsSyntax>
        ...
        var applyBody = Js.Block()
            .Add(() => Js.Const(() => "templateId", () => Js.Id(() => "document").Call(() => "getElementById", () => Js.Str(() => "templateSelect")).Prop(() => "value")))
            .Add(() => Js.If(() => new List<(JsSyntax?, List<JsSyntax>)>
            {
                { (Js.Id(() => "templateId").Op(() => "===", () => Js.Str(() => "")), new List<JsSyntax> { Js.Return(() => Js.Str(() => "")) }) }
            }))
            .Add(() => Js.Const(() => "payload", () => Js.Obj()
                .Prop(() => "beingId", () => Js.Id(() => "beingId"))
                .Prop(() => "templateId", () => Js.Id(() => "templateId"))))
            .Add(() => Js.Id(() => "fetch").Invoke(
                () => Js.Str(() => "/api/beings/tool-permissions/apply-template"),
                () => Js.Obj()
                    .Prop(() => "method", () => Js.Str(() => "POST"))
                    .Prop(() => "headers", () => Js.Obj().Prop(() => "Content-Type", () => Js.Str(() => "application/json")))
                    .Prop(() => "body", () => Js.Id(() => "JSON").Call(() => "stringify", () => Js.Id(() => "payload")))
            ).Call(() => "then", () => Js.Arrow(() => new List<string> { "r" }, () => Js.Id(() => "r").Call(() => "json")))
             .Call(() => "then", () => Js.Arrow(() => new List<string> { "data" }, () => applyDataBlock)).Stmt())
            .Add(() => Js.Assign(() => Js.Id(() => "document").Call(() => "getElementById", () => Js.Str(() => "templateSelect")).Prop(() => "value"), () => Js.Str(() => "")));
        js.Add(() => Js.Func(() => "applyTemplate", () => new List<string>(), () => applyBody));
    }
