...
[English](README.md) | [中文文档](docs/zh-CN/README.md) | [繁體中文](docs/zh-HK/README.md) | [Español](docs/es-ES/README.md) | [日本語](docs/ja-JP/README.md) | [한국어](docs/ko-KR/README.md) | [Čeština](docs/cs-CZ/README.md)
...
- **Web UI** — 内置 HTTP 服务器，支持 SSE，多种皮肤和全面的仪表板
  ...
  - **20+ 个控制器** — 关于、审计、生命体、聊天、聊天历史、代码浏览器、代码悬浮提示、配置、仪表板、执行器、初始化、知识、日志、记忆、权限、权限请求、项目、任务、定时器、定时器执行历史
  - **实时更新** — SSE（服务器发送事件）用于聊天消息、生命体状态和系统事件
  - **HTML/CSS/JS 构建器** — 通过 `H`、`CssBuilder` 和 `JsBuilder` 进行服务器端标记生成（零前端框架依赖）
  - **本地化** — 21 种内置语言变体，通过 LocalizationManager 解析
...
│       ├── Config/                        # 默认配置数据
│       ├── Localization/                  # 简体中文、繁体中文、美式英语、日语、韩语、西班牙语、捷克语、默认本地化基类、其他英语（英式英语、加拿大英语、澳大利亚英语、印度英语、新加坡英语、南非英语、爱尔兰英语、新西兰英语、马来西亚英语）、其他中文（新加坡中文、澳门中文、台湾中文、马来西亚中文）、其他西班牙语（墨西哥西班牙语）
