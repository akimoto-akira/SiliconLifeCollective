...
**版本：v0.2.0-alpha** | **硅基生命群** — 一个基于 .NET 9 的多智能体协作平台，AI 智能体被称为**硅基生命体**，通过 Roslyn 动态编译实现自我进化。

[English](../README.md) | [Deutsch](../de-DE/README.md) | **中文** | [繁體中文](../zh-HK/README.md) | [Español](../es-ES/README.md) | [日本語](../ja-JP/README.md) | [한국어](../ko-KR/README.md) | [Français](../fr-FR/README.md) | [Čeština](../cs-CZ/README.md) | [Italiano](../it-IT/README.md) | [Polski](../pl-PL/README.md) | [Português](../pt-PT/README.md) | [Русский](../ru-RU/README.md)
...
### 工具与执行
- **26 个内置工具** — 涵盖日历、聊天、配置、磁盘、网络、记忆、任务、定时器、知识库、工作笔记、项目工作区、项目任务、项目工作笔记、项目工作、WebView 浏览器、动态编译、代码执行、权限管理、Token 审计、日志查询、数据库、系统信息、帮助文档、技能管理、MCP 查询等
...
### 技能系统
- **可复用能力单元** — 把"工具编排 + 提示词模板"封装为可声明、可进化、可调度的技能，AI 像调用工具一样调用技能
- **双触发模式** — Manual（AI 函数调用自主决定）+ Auto（schedule 调度：每日定点 / 间隔周期 / cron 子集）
- **Markdown 优先** — YAML 前置元数据 + 提示词正文；纯 Markdown 保存时 AI 自动补全缺失元数据（用户字段不被覆盖）
- **热重载与版本归档** — 30 秒指纹检测自动生效；每次更新归档到 `skills/archive/{id}/{version}.md` 形成进化史
- **多重护栏** — 全局开关、配额限制（默认 50/生命体）、全局轮数与超时钳制、工具白名单、递归防护、技能级动作权限

### MCP 集成
- **外部工具接入** — 连接外部 MCP（Model Context Protocol）服务器，其工具以 `mcp_{serverId}_{toolName}` 命名自动注入所有硅基生命体，无需编写代码
- **双传输** — stdio（本地子进程）与 http（远程端点）
- **用户主权** — 服务器增删启停仅限 Web UI 操作，AI 侧 `mcp` 工具只读查询
- **权限一致** — MCP 包装工具纳入两级工具权限矩阵，可按生命体/项目禁用

### 即时通讯集成
- **多实例架构** — 可同时接入多个 IM 平台（Web UI / 飞书 / 企业微信 / 钉钉），每实例独立启停，消息聚合路由
- **OAuth 授权向导** — 飞书一键授权（state 防 CSRF、SSE 实时状态推送），令牌自动写回配置
- **密钥安全** — 配置值支持 `${ENV_VAR}` 环境变量占位符，明文密钥不落盘
...
- **多 AI 后端支持**
  ...
  - **美团 LongCat** — 美团自研大模型，LongCat-2.0 支持 1M 上下文与思考模式，兼容 OpenAI API 格式
...
- **工作流引擎** — 基于模板的状态机引擎，支持自定义工作流模板、状态转换、Tick 驱动执行和实例生命周期管理
- **记忆淡忘机制** — 定时衰减服务（MemoryFadeService），每小时自动对所有硅基生命体的记忆进行重要性衰减和自动归档

### Web 界面
- **现代化 Web UI** — 内置 HTTP 服务器，支持 SSE 实时更新
- **7 种皮肤主题** — 管理版、聊天版、创作版、开发版、高对比度、浅色、极简，支持自动发现和切换
- **27 个控制器** — 完整的系统管理、聊天、配置、技能、MCP、监控功能
- **零前端框架依赖** — 通过 `H`、`CssBuilder` 和 `JsBuilder` 在服务端生成 HTML/CSS/JS

### 国际化与本地化
- **34 种语言变体**全面支持，涵盖 2 种书写系统和多个地区变体
...
- [x] 阶段 8：长期记忆 + 任务 + 定时器
- [x] 阶段 9：核心主机 + 多智能体协作
- [x] 阶段 10：Web UI（HTTP + SSE，27 个控制器，7 种皮肤）
...
- [x] 阶段 12.5：国内 AI 平台扩展（DeepSeek / 智谱 GLM / Kimi / 硅基流动 / MiniMax / 千帆 ERNIE / 腾讯混元，共 13 个 AI 客户端）
- [x] 阶段 13：外部即时通讯集成（多实例架构：飞书 / 企业微信 / 钉钉，飞书 OAuth 授权向导）
- [x] 阶段 13.5：技能系统（工具编排 + 提示词模板、双触发模式、热重载、版本归档）+ MCP 集成（外部服务器工具接入、Web 管理页面）
