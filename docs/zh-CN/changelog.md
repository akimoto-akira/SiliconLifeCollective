...
- 重构 PermissionTool 使用真实权限评估
  - 移除无功能的 `grant` 和 `revoke` 动作
  - `query_permission` 现在使用 `EvaluatePermission` API，参数为 `permission_type` 和 `resource`
  - 返回三态结果（ALLOWED、DENIED、ASK_USER）及详细上下文
- 重构 ExecuteCodeTool 为纯编译执行
  - 移除解释器代码执行（Python、Node.js 等）
  - 移除包安装功能
  - 简化为仅支持 C# 编译加安全扫描
  - 改进编译错误和安全违规报告
...
- CodeEditorView 和 MarkdownEditorView 中的 JsBuilder 错误处理改进
  - 增强的客户端错误显示，提供详细错误消息
  - 更好的 HTTP 错误处理和状态码报告
- 权限控制器 API 路由注册
  - 添加 `/api/permissions/list` 路由
  - 添加 `/api/permissions/save` POST 路由
...
- 权限保存工作流重构
  ...
  - 改进安全扫描集成
  - 增强所有错误消息的本地化支持
- CodeEditorView 和 MarkdownEditorView 保存响应处理
  - 更详细的成功/错误反馈
  - 改进包含错误详情的警报消息
