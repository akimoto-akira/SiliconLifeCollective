...
[English](../en/architecture.md) | [Deutsch](../de-DE/architecture.md) | **中文** | [繁體中文](../zh-HK/architecture.md) | [Español](../es-ES/architecture.md) | [日本語](../ja-JP/architecture.md) | [한국어](../ko-KR/architecture.md) | [Čeština](../cs-CZ/architecture.md) | [Русский](../ru-RU/architecture.md)
...
- **角色说明**：作为架构验证的基准实现，提供简单可靠的运行方式，适合初次接触本项目、进行开发调试或数据安全优先的场景

### SiliconLife.Fast（高性能版本）
- **定位**：主推生产版本
- **运行模式**：桌面应用程序（Windows 系统托盘 / Linux 状态窗口）
- **存储方式**：SpeedyPack 内存存储 + 异步批量持久化（.spk 文件格式）
- **适用场景**：高并发、低延迟、大数据量场景
- **平台支持**：Windows/macOS（完整功能，含系统托盘）、Linux（状态窗口，无托盘图标）
- **特点**：
  - Windows/macOS 系统托盘后台运行，托盘状态窗口实时监控；Linux 状态窗口直接显示
...
| 美团 LongCat | ✅ | 云端 | 美团自研大模型，LongCat-2.0 支持 1M 上下文与思考模式，兼容 OpenAI API 格式 |
...
Web UI 遵循**类 MVC 模式**，24 个控制器处理不同方面：
...
| Project | 项目管理，包含工作笔记、任务系统和工具权限 |
| System | 系统管理与运行时监控 |
| Task | 任务系统界面 |
| Timer | 定时器系统管理，包含执行历史 |
| ToolPermission | 工具权限管理，支持硅基生命体和项目级别的权限配置 |
...
- **其他（8 种）**：ja-JP（日语）、ko-KR（韩语）、cs-CZ（捷克语）、it-IT（意大利语）、pl-PL（波兰语）、pt-PT（葡萄牙语）、pt-BR（巴西葡萄牙语）、ru-RU（俄语）
...
工作流引擎是基于模板的状态机系统，用于驱动硅基生命体在项目空间中的协作流程：

### 核心组件

| 组件 | 描述 |
|------|------|
| `WorkflowEngine` | 工作流引擎核心，管理模板和实例，执行 Tick 驱动的状态转换 |
| `WorkflowTemplate` | 工作流模板，定义状态集合和转换规则 |
| `WorkflowInstance` | 工作流实例，绑定到具体项目，跟踪当前状态 |
| `WorkflowLog` | 工作流日志，记录状态转换历史 |

### 工作机制

- **模板注册**：通过 `RegisterTemplate()` 注册工作流模板，定义状态和转换规则
- **实例创建**：从模板创建实例，绑定到项目空间
- **Tick 驱动**：状态转换由主循环的 Tick 机制驱动
- **日志记录**：所有状态转换自动记录到日志
...
`MemoryFadeService` 是一个定时衰减服务，模拟生物记忆的遗忘特性：

### 工作机制

- **定时执行**：继承自 `TickObject`，默认每小时执行一次衰减周期
- **重要性衰减**：对每个硅基生命体的记忆条目应用衰减算法，降低重要性评分
- **自动归档**：重要性低于阈值的记忆自动归档（`ArchiveFadingMemories()`）
- **统计追踪**：记录衰减周期数、状态变更条目数等统计数据
...
项目工作区是支持多硅基生命体协作的空间管理机制：

### 核心功能

- **项目生命周期**：创建 → 活跃 → 归档 → 销毁
- **角色分配**：支持为硅基生命体分配项目角色
- **工具权限隔离**：项目级别的工具权限配置，独立于硅基生命体级别的权限
- **工作笔记**：项目空间内的页面式笔记系统，支持目录生成和关键词搜索
- **任务追踪**：项目级别的任务管理，支持创建、分配、状态跟踪
- **工作流集成**：项目可绑定工作流模板，驱动协作流程

### 相关工具

| 工具 | 用途 |
|------|------|
| `ProjectTool` | 项目空间管理（创建、归档、销毁、角色分配） |
