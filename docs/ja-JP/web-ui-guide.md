...
- **ビュー**：
  - `ChatHistoryListView` - 会話リスト
  - `ChatHistoryDetailView` - メッセージ詳細
- **APIルート**：
  - `/api/chat-history/{beingId}/conversations` - 会話リスト取得
...
- テキストファイル（.txt、.md、.json、.xmlなど）
- コードファイル（.cs、.js、.py、.javaなど）
- 設定ファイル（.yml、.yaml、.ini、.confなど）
- ドキュメントファイル（.csv、.logなど）
...
- チャットページのローディング状態表示
- キュレーターセッションの自動選択
- データローディング進行状況のフィードバック

### 動作

- ページロード時にローディングアニメーションを表示
- データロード完了後に自動的に非表示
- キュレーターセッションが自動的に選択される（存在する場合）
- 多言語ローディングプロンプトテキスト
