...
- **実行器-権限セキュリティ** — すべての I/O 操作は実行器を通じて厳格な権限検証を行う
  - 3段階分岐権限チェーン：UserFrequencyCache → IPermissionCallback → (IsCurator: IPermissionAskHandler | 非主理人: GlobalACL)
  - 完全な監査ログがすべての権限決定を記録
...
- **23 のコントローラー** — 完全なシステム管理、チャット、設定、モニタリング機能
- **ゼロフロントエンドフレームワーク依存** — `H`、`CssBuilder`、`JsBuilder` を介してサーバーサイドで HTML/CSS/JS を生成

### 国際化とローカライゼーション
- **33 種類の言語変種**を完全にサポートし、2 種類の書体システムと複数の地域バリアントをカバー
  ...
  - **イタリア語**：it-IT | **ポーランド語**：pl-PL | **ポルトガル語**：pt-PT, pt-BR（4 種類）
...
- **特徴**：
  ...
  - Component UI アーキテクチャ、27個の宣言的コンポーネント
...
│   │   ├── Localization/                  # ローカライゼーションベースクラスと33種類の言語/地域バリアント実装
│   │   ├── Resources/                     # 共有リソースファイル
│   │   ├── Security/                      # 権限マネージャー
│   │   ├── SiliconBeing/                  # デフォルトシリコン生命体実装
│   │   ├── Tools/                         # 24個の共通ツール実装（ホットリロードツール含む）
│   │   ├── Web/                           # Web 基盤
...
│   │   └── Web/                           # Web UI 実装
│   │       ├── Component/                 # UI コンポーネントライブラリ（27 コンポーネント）
│   │       ├── Controllers/               # 23 のコントローラー
│   │       ├── Models/                    # ビューモデル
│   │       ├── Views/                     # HTML ビュー
│   │       └── Skins/                     # 7 種類のスキンのテーマ
│   │
...
```
ツール呼び出し → 実行器 → 権限マネージャー → [周波数キャッシュ → コールバック → (主理人: ユーザーに確認 | 非主理人: グローバルACL)]
```
...
- [x] フェーズ 10：Web UI（HTTP + SSE、23 コントローラー、7種類のスキン）
- [x] フェーズ 10.5：増分強化（ブロードキャストチャンネル、トークン監査、32種類のカレンダー、ツール強化、33言語ローカライゼーション）
