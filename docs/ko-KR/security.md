...
실리콘 큐레이터는 최고 권한 수준(`IsCurator = true`)을 가집니다. 큐레이터의 권한 검사는 사용자가 명시적으로 재정의하지 않는 한 **허용**으로 단락됩니다.
...
쿼리 우선순위: **1. UserFrequencyCache → 2. IPermissionCallback → 3. Curator 분기 (AskHandler/GlobalACL)**
