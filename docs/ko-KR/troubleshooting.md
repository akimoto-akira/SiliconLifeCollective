...
[English](../en/troubleshooting.md) | [Deutsch](../de-DE/troubleshooting.md) | [中文](../zh-CN/troubleshooting.md) | [繁體中文](../zh-HK/troubleshooting.md) | [Español](../es-ES/troubleshooting.md) | [日本語](../ja-JP/troubleshooting.md) | **한국어** | [Čeština](../cs-CZ/troubleshooting.md) | [Русский](../ru-RU/troubleshooting.md)
...
**해결책**:
1. API 키가 올바른지 확인
2. 모델 이름이 바이리안 카탈로그와 일치하는지 확인
3. 지역 엔드포인트가 올바른지 확인
4. 계정에 모델 접근 권한이 있는지 확인

#### 문제: Volcengine Ark 연결 실패
...
**해결책**:
1. API 키가 올바른지 확인
2. Endpoint URL 형식이 올바른지 확인 (기본값: `https://ark.cn-beijing.volces.com/api/v3/chat/completions`)
...
2. 스토리지 무결성 확인:
```bash
# Web UI 통해: 시스템 > 스토리지 확인
```

3. 손상된 파일 수동으로 수정

#### 문제: SpeedyPack 스토리지 파일 손상 (Fast 버전)
...
#### 문제: SpeedyPack 자동 컴팩션 실패 (Fast 버전)
...
2. 수동으로 컴팩션 작업 트리거
3. 컴팩션 임계값 구성 확인
4. `SiliconLife.Speedy.Manager` 도구를 사용하여 수동 컴팩션
...
**증상**:
- 플러그인이 로딩되었지만 툴이 툴 목록에 나타나지 않음

**해결책**:
1. 플러그인의 툴 클래스가 `ITool` 인터페이스를 올바르게 구현했는지 확인
2. 툴 클래스가 public인지 확인
...
**해결책**:
1. 비잉이 존재하고 실행 중인지 확인
2. 스토리지 경로에 쓰기 권한이 있는지 확인
...
**해결책**:
1. 두 개념이 노리지 네트워크에 존재하는지 확인
2. 연결 경로가 있는지 확인 (직접적 또는 간접적 관계가 없을 수 있음)
3. 더 많은 노리지를 추가하여 연결 구축 시도
...
#### 문제: 노리지 네트워크 통계 정보가 부정확

**증상**:
- 통계 숫자가 예상과 다름
- 노리지 추가 후 통계가 업데이트되지 않음
...
**해결책**:
1. 실리콘 비잉이 프로젝트에 참여했는지 확인
2. 역할 이름이 유효한지 확인
3. 작업자가 실리콘 큐레이터인지 확인
4. 자세한 오류 정보에 대한 로그 확인
...
**해결책**:
1. 워크플로 템플릿이 정의되었는지 확인
2. 초기 상태가 올바르게 설정되었는지 확인
3. 프로젝트에 워크플로 템플릿이 바인딩되었는지 확인
4. 전환 오류에 대한 워크플로 로그 확인
...
3. 또는 Web UI 사용: 비잉 → 툴 권한

#### 문제: 프로젝트 툴 권한이 적용되지 않음

**증상**:
- 프로젝트 수준의 툴 권한이 예상대로 작동하지 않음

**해결책**:
1. 프로젝트 수준의 권한이 올바르게 구성되었는지 확인
2. 실리콘 비잉 수준과 프로젝트 수준 권한 간 충돌이 있는지 확인
3. 프로젝트 수준 권한은 실리콘 비잉 수준과 독립적이며, 두 권한의 교집합이 적용됨
4. 감사 로그에서 권한 확인 결과 확인
