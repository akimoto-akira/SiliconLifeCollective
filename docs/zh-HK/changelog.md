...
#### 文件一致性修復
- `9e07b27` - 修正法語文件(fr-FR)與源代碼一致性差異 (ref task-307)
  - 10 個文件變更

- `9e3be72` - 修正德語文件(de-DE)與源代碼的一致性 (ref task-308)
  - 5 個文件變更

- `2bc7151` - 修正西班牙語(es-ES)文件與源代碼一致性差異 (ref task-309)
  - 13 個文件變更

- `f95088e` - 修正意大利語文件(it-IT)與源代碼的一致性 (ref task-310)
  - 11 個文件變更

- `6ea9f4a` - 修正波蘭語文件(pl-PL)與源代碼的一致性 (ref task-311)
  - 16 個文件變更

- `7646923` - 修正葡萄牙語文件(pt-PT)與源代碼的一致性 (ref task-312)
  - 12 個文件變更

- `7eaf9db` - 修正捷克語(cs-CZ)文件與源代碼的一致性 (ref task-313)
  - 12 個文件變更
...
#### 新功能
- `99eca78` - 右鍵選單添加「查看存儲（唯讀）」功能，進程內調用 Speedy.Manager (ref task-301)
  - 26 個文件變更

#### 文件一致性修復
- `7f65cf1` - 修正 zh-CN 文件與源代碼的一致性差異 (ref task-303)
  - 15 個文件變更

- `a9e2a2c` - 修正英文(en)文件與源代碼一致性差異 (ref task-302)
  - 9 個文件變更

- `2549105` - 修正繁體中文(zh-HK)文件與源代碼一致性差異 (ref task-304)
  - 12 個文件變更

- `277eb50` - 修正日語文件與源代碼的整合性差異 (ref task-305)
  - 10 個文件變更

- `edce413` - 修正韓語(ko-KR)文件與源代碼的一致性差異 (ref task-306)
  - 18 個文件變更

- `f2adcae` - 修復葡萄牙語文件與源代碼不一致問題 (ref task-220)
  - 15 個文件變更

- `3332987` - 修復繁體中文（香港）文件與源代碼不一致問題 (ref task-218)
  - 14 個文件變更

- `af9f715` - 修復波蘭語文件與源代碼不一致問題 (ref task-217)
  - 15 個文件變更

- `2e2b18b` - 修復韓語文件與源代碼不一致問題 (ref task-216)
  - 16 個文件變更

- `626ebc9` - 修復日語文件與源代碼不一致問題 (ref task-215)
  - 19 個文件變更

- `48d061b` - 修復意大利語文件與源代碼不一致問題 (ref task-214)
  - 14 個文件變更
...
#### 新功能
- `65176d4` - 添加葡萄牙語（pt-PT + pt-BR）完整本地化支持 (ref task-208)
  - 41 個文件變更

#### 文件一致性修復
- `af4dffd` - 修正 zh-CN 文件與源代碼的全部不一致問題 (ref task-209)
  - 11 個文件變更

- `144b945` - 修正英文(en)和捷克語(cs-CZ)文件與源代碼不一致問題 (ref task-219, task-210)
  - 22 個文件變更

- `08bec55` - 修正德語文件(de-DE)與源代碼不一致問題 (ref task-211)
  - 14 個文件變更

- `7ff28de` - 修復西班牙語(es-ES)文件與源代碼不一致問題 (ref task-212)
  - 14 個文件變更

- `15e2133` - 修復法語文件(fr-FR)與源代碼不一致問題 (ref task-213)
  - 13 個文件變更

#### 修復
- `7dac388` - 修復項目任務列表無法顯示 (ref task-207)
  - 6 個文件變更
...
#### 新功能
- `cd72846` - 實現 PluginLoader 安全掃描繞過的安全替代方案 (ref task-203)
  - 13 個文件變更

- `fc0c00c` - Speedy.Manager 功能增強 - 新建/導入/導出/TreeView 層級/進度窗口 (ref task-206)
  - 9 個文件變更

#### 修復
- `ec07118` - 修復 ITypeRegistry/IObjectFactory 未在插件加載前註冊的問題 (ref task-205)
  - 8 個文件變更

- `9e749db` - 修復創建項目時 Creator ID is required 錯誤 (ref task-204)
  - 4 個文件變更

#### 基礎設施
- `43dc092` - CLDR 遷移 - 添加 CldrDataProvider，移除 .github
  - 1 個文件變更

- `c09ec1f` - 添加 cldr/ 到 .gitignore
  - 1 個文件變更

- `221f818` - GitHub 同步改用 Gitee 推送鏡像方案，workflow 僅保留手動備用
  - 1 個文件變更

- `08cdf1a` - 修復 GitHub 同步 workflow - 添加重試邏輯和無變更跳過
  - 1 個文件變更
...
#### 新功能
- `d6eb994` - 項目列表頁增加創建項目入口和工作流模板選擇 (ref task-203)
  - 14 個文件變更

- `0872134` - ThinkOnProject 無模板項目主理人驅動編排 (ref task-202)
  - 6 個文件變更

- `cb3188e` - 群聊 @提及可視化 (ref task-208)
  - 4 個文件變更

- `f9968e5` - AI 客戶端 ToolCall 能力聲明與優雅降級 (ref task-205)
  - 4 個文件變更

- `0d2b843` - 群聊決策邏輯 ShouldReplyInGroupChat (ref task-201)
  - 6 個文件變更

- `277a2b1` - 知識網絡補全 - 高級查詢和圖譜遍歷 (ref task-207)
  - 9 個文件變更

#### 修復
- `6d0b66e` - 修復群聊發送消息時 appendMessage TypeError (ref task-209)
  - 5 個文件變更

- `b15167c` - 補充提交 task-203 遺漏的 list-workflow-templates 路由註冊 (ref task-203)
  - 1 個文件變更

- `dc549a2` - 修復 Gitee 同步 workflow - 添加用戶名到 token URL
  - 1 個文件變更

#### 基礎設施
- `e5fa3ad` - 禁用 GitHub 自動同步 schedule，等待 Gitee 官方同步方案
  - 1 個文件變更

#### 協作框架
- `4a58c82` - 新增系统能力分析報告 + ThinkOnProject 設計方案
  - 5 個文件變更

- `8ab29e6` - 系统能力完整性分析報告歸檔至 .ai-collab/docs
  - 2 個文件變更

- `b412d9c` - 歸檔舊任務，基於綜合分析重新發佈 task-201~208
  - 2 個文件變更

- `437884a` - 更新協作元數據 - task-202/203/204 完成 (ref task-202, task-203, task-204)
  - 2 個文件變更

- `bf78d79` - 更新協作元數據 - task-201/205/208 完成
  - 2 個文件變更
...
#### 文件更新
- `aa1d2ad` - 更新全部 11 種語言 README/架構/入門文件，體現 SiliconLife.Fast 多平台支援 (ref task-112, task-113)
  ...
  - ConfigView 列舉下拉框傳入 hint 參數
  - 11 種語言本地化新增 SelectSearchHint 鍵值
  - 53 個檔案變更，690 行新增，194 行刪除

#### 任務系統
- `3329f3d` - 新增任務系統巡檢機制 + 本地化錯誤修復任務
  - 建立 task-113：修復關於頁面本地化問題
  - 更新 task-112：更新 Fast 版本文件支援 Linux
  - 歸檔已完成任務（11 個）至 .ai-collab/archive/
  - 巡檢機制設定完成：快速巡檢（每 30 分鐘）+ 全量巡檢（每天 06:00）
  - 2 個檔案變更，148 行新增，171 行刪除

#### 協作框架
- `6038e22` - 註冊 coze-agent 至 .ai-collab 協作註冊表
  - 新增扣子平台常駐 AI 註冊資訊
  - 1 個檔案變更
...
#### AI 協作框架
- `7344fbb` - 移除 handoff 模式，改為任務列表驅動 (v2.0)
  - 重構 .ai-collab 目錄結構，從 handoff 交接模式改為任務列表驅動
  - 新增 tasks.json 任務列表核心檔案
...
- `5481bcf` - 註冊 Qoder AI IDE 至協作註冊表
...
- `ec176b2` - 覆蓋任務列表 - 程式碼審查發現 10 個新錯誤
  - 建立 task-101~110 共 10 個新任務

#### 重構
- `ab15915` - 統一版權標頭 + 修復 HelpController BOM 和 HelpView 搜尋 JS
  - 統一所有 C# 原始檔 Apache 2.0 版權標頭
...
- `9eb251a` - 移除 SiliconLife.McpServer 模組 (ref task-111)
  - 移除獨立 MCP 伺服器，功能已整合至主專案

### 2026-05-13

#### 本地化
- `7a62590` - 新增波蘭語本地化支援
  - 新增 pl-PL 波蘭語本地化實作（PlPL.cs，1089 行）
  - 新增波蘭語幫助文件本地化（HelpLocalizationPlPL.cs，3972 行）
  - 新增波蘭語中國歷史日曆支援（ChineseHistoricalPlPL.cs，600 行）
  - 新增波蘭語系統匣本地化（TrayPlPL.cs，135 行）
  - 新增波蘭語完整文件集（15 個文件）
  - Language 列舉新增波蘭語
  - 35 個檔案變更，14379 行新增，11 行刪除

- `51f9c8e` - 更新文件中的 Ark AI 參照和術語改進
  - 更新多語言文件中的 AI 用戶端術語
