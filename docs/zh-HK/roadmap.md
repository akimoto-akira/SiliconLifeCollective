...
- **定位**：主推生產版本
- **當前狀態**：已完成基礎架構移植，SpeedyPack 儲存引擎和插件系統已實作
...
| # | 模組 | 描述 |
|---|--------|-------------|
| 1.1 | 解決方案和項目結構 | 建立 `SiliconLifeCollective.sln`，包含 `src/SiliconLife.Core/`（核心庫）和 `src/SiliconLife.Default/`（預設實作 + 入口點） |
...
| 10.9–10.17 | Web 控制器 | 聊天、儀表板、生命體、任務、權限、權限請求、執行器、日誌、設定、記憶、定時器、初始化、關於、程式碼瀏覽器、知識、項目、審計 |
...
| 10.6.3 | ProjectWorkNoteTool | 項目工作筆記工具，支援項目維度的工作記錄和管理 |
| 10.6.4 | ProjectTaskTool | 項目任務管理工具，支援任務分配、進度追蹤 |
...
**交付物**：完整的 WebView 瀏覽器自動化、幫助文件系統、項目工作區、知識網絡、聊天歷史檢視等增強功能。

**驗證**：矽基生命體可以透過 WebViewBrowserTool 操作瀏覽器 → 透過 HelpTool 取得幫助文件 → 管理項目工作筆記和任務 → 查詢知識網絡 → 檢視聊天歷史。

---

## ~~階段 10.7：項目協作與工作流~~ ✅ 已完成

**目標**：新增項目工作區、工作流引擎、記憶淡忘機制和工具權限系統。

| # | 模組 | 描述 |
|---|--------|-------------|
| 10.7.1 | 項目角色管理 | ProjectTool 新增 assign_role、remove_role、list_roles 操作 |
| 10.7.2 | 工作流引擎 | WorkflowEngine 核心引擎，支援模板定義、狀態轉換、Tick 驅動執行 |
| 10.7.3 | 工作流模板 | WorkflowTemplate 基底類別，定義狀態集合和轉換規則 |
| 10.7.4 | 工作流實例 | WorkflowInstance 實例管理，綁定到具體項目，追蹤當前狀態 |
| 10.7.5 | 工作流日誌 | WorkflowLog 記錄狀態轉換歷史 |
| 10.7.6 | 記憶淡忘機制 | MemoryFadeService 定時衰減服務，每小時自動對記憶進行重要性衰減和歸檔 |
| 10.7.7 | 工具權限系統 | 兩級工具權限（矽基生命體級別 + 項目級別），權限模板，操作粒度控制 |
| 10.7.8 | ToolPermissionController | 工具權限管理 Web 控制器 |
| 10.7.9 | ProjectWorkTool | 項目工作操作工具（[SiliconManagerOnly]，[ToolScenario(Project)]） |
...
**交付物**：完整的項目協作系統、工作流引擎、記憶淡忘機制和工具權限管理。

**驗證**：建立項目 → 分配角色 → 綁定工作流模板 → 生命體在項目空間內協作 → 記憶自動衰減歸檔 → 工具權限隔離生效。
...
| 12.2 | ~~插件系統~~ ✅ 已完成 | 外部插件載入，帶安全檢查和沙箱（IPlugin 介面、PluginLoader、AssemblyLoadContext 隔離） |
