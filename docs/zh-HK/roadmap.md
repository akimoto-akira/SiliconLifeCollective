...
- **角色說明**：作為架構驗證的基準實現，確保核心架構設計的正確性和可行性

### SiliconLife.Fast（高效能版本）
- **定位**：主推生產版本
- **當前狀態**：已完成基礎架構移植，SpeedyPack 儲存引擎和外掛程式系統已實現
- **角色說明**：在 Default 版本驗證的架構基礎上，進行深度效能優化和生產特性增強，是實際部署的首選
...
**目標**：控制臺輸入 → AI 呼叫 → 控制臺輸出。最小可驗證單元。

| # | 模組 | 描述 |
|---|--------|-------------|
| 1.1 | 解決方案和專案結構 | 建立 `SiliconLifeCollective.sln`，包含 `src/SiliconLife.Core/`（核心程式庫）和 `src/SiliconLife.Default/`（預設實現 + 入口點） |
| 1.2 | 設定（最小化） | 單例 + JSON 反序列化。讀取 `config.json`。如果缺失則自動生成預設值 |
| 1.3 | 在地化（最小化） | `LocalizationBase` 抽象類別，`ZhCN` 實現。在設定中添加 `Language` |
| 1.4 | OllamaClient（最小化） | `IAIClient` 介面，HTTP 呼叫本地 Ollama `/api/chat`。尚無流式傳輸，尚無工具呼叫 |
| 1.5 | 控制臺 I/O | `while(true) + Console.ReadLine()`，讀取輸入 → 呼叫 AI → 列印回應 |
...
**交付物**：由主迴圈驅動程式的矽基生命體，接收控制臺輸入，載入靈魂檔案，呼叫 AI。
...
| 5.3 | IAIClient：工具呼叫支援 | 解析 AI tool_calls。迴圈：執行工具 → 發送結果回去 → AI 繼續 → 直到純文字 |
...
| 6.5 | UserFrequencyCache | HighAllow/HighDeny 列表。使用者選擇（非自動檢測）。前綴匹配，僅記憶體，可設定過期 |
...
| 7.6 | DefaultCurator | 預設館長實現，帶內建靈魂檔案和管理工具 |
...
**交付物**：矽基生命體可以透過 AI 生成新程式碼，編譯並替換自己。
...
| 10.3–10.5 | HtmlBuilder / CssBuilder / JsBuilder | C# 伺服器端建構器。零前端架構依賴 |
...
| 10.5.11 | DashScopeClient | 阿里雲百煉 AI 客戶端，相容 OpenAI API。支援流式傳輸、工具呼叫、推理內容 |
...
| 11.3 | TelegramProvider | Telegram Bot API 集成，支援內聯鍵盤 |
