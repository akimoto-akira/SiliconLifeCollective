...
[English](../README.md) | [中文](../zh-CN/README.md) | **繁體中文** | [Deutsch](../de-DE/README.md) | [Español](../es-ES/README.md) | [日本語](../ja-JP/README.md) | [한국어](../ko-KR/README.md) | [Français](../fr-FR/README.md) | [Čeština](../cs-CZ/README.md) | [Italiano](../it-IT/README.md) | [Polski](../pl-PL/README.md) | [Português](../pt-PT/README.md) | [Русский](../ru-RU/README.md)
...
- **7 種皮膚主題** — 管理版、聊天版、創作版、開發版、高對比度、淺色、極簡，支援自動發現和切換
- **20+ 個控制器** — 23 個控制器，完整的系統管理、聊天、配置、監控功能
...
- **33 種語言變體**全面支援，涵蓋 4 種書寫系統（拉丁、CJK、阿拉伯、天城文）和多個地區變體
  ...
  - **意大利語**：it-IT | **波蘭語**：pl-PL | **葡萄牙語**：pt-PT, pt-BR（4 種）
...
- **角色說明**：作為架構驗證的基準實現，適合初次接觸、開發除錯或資料安全優先的場景
...
|------|---------------------|------------------|
| 執行時 | .NET 9 | .NET 9（Windows/macOS/Linux） |
| 程式語言 | C# | C# |
| 應用類型 | 控制台應用程式 | 桌面應用程式（Windows/macOS 系統匣 / Linux 狀態視窗） |
| AI 整合 | Ollama（本地）、阿里雲百煉（雲端）、火山引擎Ark（雲端） | Ollama（本地）、阿里雲百煉（雲端）、火山引擎Ark（雲端） |
| 資料儲存 | 檔案系統（JSON + 時間索引目錄） | SpeedyPack（.spk 格式，記憶體對映 + 非同步持久化） |
...
| 系統匣 | ❌ 不支援 | ✅ Windows/macOS 支援（NotifyIcon）；Linux 無匣圖示 |
...
│   │   ├── Calendar/                      # 32 種日曆實現
│   │   ├── Localization/                  # 本地化基類與 33 種語言/地區變體實現
│   │   ├── Resources/                     # 共享資源檔案
│   │   ├── Security/                      # 權限管理器
│   │   ├── SiliconBeing/                  # 預設矽基生命體實現
│   │   ├── Tools/                         # 24 個通用工具實現（含熱重載工具）
│   │   ├── Web/                           # Web 基礎設施
│   │   └── WebView/                       # Playwright WebView 實現
│   │
│   ├── SiliconLife.App/                   # 應用層（Web UI + 幫助文件，Default 與 Fast 共享）
│   │   ├── Config/                        # 應用配置
│   │   ├── Data/                          # 資料目錄
│   │   ├── Help/                          # 幫助文件本地化（多語言）
│   │   └── Web/                           # Web UI 實現
...
- [x] 階段 8：長期記憶 + 任務 + 定時器
- [x] 階段 9：核心主機 + 多智能體協作
- [x] 階段 10：Web UI（HTTP + SSE，23 個控制器，7 種皮膚）
- [x] 階段 10.5：增量增強（廣播頻道、Token 審計、32 種日曆、工具增強、33 種語言變體本地化）
- [x] 階段 10.6：完善與最佳化（WebView、幫助系統、專案工作區、知識網路）
- [x] 階段 11：SpeedyPack 儲存引擎（替換 LiteDB、記憶體對映、非同步寫入佇列、自動壓縮）
- [x] 階段 12：插件系統（IPlugin 介面、PluginLoader 安全沙箱、隔離載入、工具整合）

### 🚧 計劃中
- [ ] 階段 13：外部即時通訊整合（飛書 / WhatsApp / Telegram）
- [ ] 階段 14：技能生態系統（插件市場、技能包分發）
...
**SiliconLife.Default（預設實現 — 驗證架構可行性）：**
- � 您第一次接觸本專案，希望快速了解系統架構
- 📌 您正在進行開發除錯，需要簡單直接的運行方式
- 📌 資料安全性是您的首要考慮
- 📌 您的系統記憶體小於 4GB
- 📌 您只需要單人使用或資料量較小
...
```
開發/驗證環境：SiliconLife.Default（驗證架構、除錯功能）
生產環境：SiliconLife.Fast（高效能、後台運行、處理即時請求）
```

**策略 2：Fast 主運行，Default 定期備份**
```
