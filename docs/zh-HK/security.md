...
目前的執行器實作（`DiskExecutor`、`NetworkExecutor`、`CommandLineExecutor`）是**靜態類別**，提供帶逾時控制的同步執行：

- 每個執行器在執行前透過呼叫者的 `PermissionManager` 檢查權限。
- 操作在 `Task.Run` 上執行，具有可設定的逾時。
- 逾時時，操作被視為失敗。
- `ExecutorBase` 提供帶有背景執行緒和請求佇列支援的抽象基底類別，用於未來擴展。
...
1. 執行器接收請求。
2. 執行器透過 `ServiceLocator.Instance.GetPermissionManager(callerId)` 查詢生命體的私有權限管理器。
3. 如果權限被拒絕，操作立即被阻止。
4. 如果回呼回傳詢問使用者（主理人路徑），執行器執行緒**保持鎖定**等待使用者回應。
5. 生命體只看到最終結果（成功或拒絕） —— 它永遠不會看到中間的「待處理」或「等待」狀態。
6. 只有矽基主理人會觸發真正的使用者提示。普通生命體同步查詢全域 ACL 而不阻塞。
7. 逾時時，請求被視為拒絕，執行緒鎖被釋放。
...
| `NetworkExecutor` | HTTP 請求、WebSocket 連線 | 30 秒 |
| `CommandLineExecutor` | Shell 命令執行 | 30 秒 |
| `DynamicCompilationExecutor` | Roslyn 記憶體編譯 | N/A（委託給 CompilationCore） |
...
- 變更立即生效。
- 全域 ACL 由 `PermissionManager` 在回呼返回 AskUser 或未配置回呼時，為**非主理人**生命體直接檢查。它**不是**由回呼函式引用的。
