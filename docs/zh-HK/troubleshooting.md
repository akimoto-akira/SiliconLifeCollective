...
**解決方案**：
1. 檢查插件是否參照了禁止的命名空間（`System.IO`、`System.Net.Http`、`System.Net.WebSockets`、`System.Net.Sockets`、`Microsoft.CodeAnalysis`）
2. 驗證插件只參照了可信程式集白名單中的程式集
3. 檢查插件是否正確實作 `IPlugin` 介面
4. 檢視日誌獲取詳細的安全檢查失敗原因
...
**解決方案**：
1. 確認項目級別的權限已正確設定
2. 檢查矽基生命體級別和項目級別權限是否衝突
3. 項目級別權限獨立於矽基生命體級別，兩者取交集
4. 檢視審計日誌確認權限檢查結果
